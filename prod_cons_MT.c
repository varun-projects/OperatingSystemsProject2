...
File Name         : prod_cons_MT.c  , part of Module prod_cons_MT
...
extern queue Queue; 
extern int BUFFER_SIZE,CONS_NUM;
int Data_Counter=0;
...
void *Consumer_thread( void * ptr ){
  ...
  if (ThreadID_C == (CONS_NUM-1))loopCount_C = last_cons;
  printf("C%d: Consuming %d values\n",ThreadID_C, loopCount_C);  // debugging 
 
 sleep(1);
for(int C0=0; C0<loopCount_C; C0++){      // infinte loop  -> make it "for loop" to run  loopCount_P  times
...
int randNum(void ) {
  srand(clock());                  // seed random number generator from system clcok ticks
